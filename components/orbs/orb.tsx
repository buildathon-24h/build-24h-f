...
export function Orb({
  ...
  // Precedence: explicit colors > colorsRef (live, per-frame in Scene) >
  // preset variant by agentState > default blueGreen preset.
  const resolved =
    colors ?? resolveOrbColors(preset ?? DEFAULT_ORB_PRESET, agentState)
  return (
    <div className={className ?? "relative h-full w-full"}>
      <Canvas
        ...
        gl={{
          alpha: true,
          antialias: true,
          // Shader outputs straight (non-premultiplied) alpha. Keeping this
          // false avoids a rectangular halo around the circle that flickers
          // over the ambient background gradient.
...
void main() {
    ...
    vec3 normal = normalize(vec3(uv.x * 0.82, uv.y * 0.82, sphereZ));
    vec3 lightDir = normalize(vec3(-0.45, 0.68, 0.58));

    vec2 flowUv = uv * 0.42 + vec2(uAnimation * 0.025, -uAnimation * 0.018);
    float flowNoise = texture(uPerlinTexture, flowUv).r;
    float cloudNoise = texture(uPerlinTexture, uv * 0.18 + vec2(-uAnimation * 0.012, uAnimation * 0.016)).r;
    float grain = texture(uPerlinTexture, uv * 2.6 + vec2(uTime * 0.035, -uTime * 0.026)).r - 0.5;
    ...
    vec3 body = mix(uColor1 * 1.05, uColor2 * 1.18, colorMix);

    float diffuse = clamp(dot(normal, lightDir), 0.0, 1.0);
    float rim = pow(max(0.0, 1.0 - sphereZ), 2.0);
    float topLeftHighlight = smoothstep(0.48, 0.02, distance(uv, vec2(-0.34, 0.46)));
    float lowerGlow = smoothstep(0.42, 0.0, distance(uv, vec2(-0.42, -0.55)));
    float lowerShadow = smoothstep(0.12, 0.92, distance(uv, vec2(-0.15, 0.16))) * smoothstep(-0.1, -0.78, uv.y + uv.x * 0.18);

    vec3 color = body;
    color *= 0.76 + diffuse * 0.36;
    color = mix(color, vec3(0.82, 0.96, 1.0), topLeftHighlight * 0.72);
    color = mix(color, vec3(0.72, 0.96, 1.0), lowerGlow * 0.38);
    color *= 1.0 - lowerShadow * 0.36;
    color = mix(color, vec3(1.0), rim * 0.22);

    float talkingEnergy = smoothstep(0.45, 0.9, uOutputVolume);
    color += vec3(0.06, 0.12, 0.12) * talkingEnergy * (0.4 + flowNoise * 0.6);
    color += grain * 0.16;

    // Keep the orb luminous in dark mode instead of flipping it into dark wedges.
    color *= mix(1.0, 1.08, uInverted);
